...
This document is based on the [NetLogo's Programming Guide](https://ccl.northwestern.edu/netlogo/docs/programming.html).
...
Patches are `matrix` objects with two columns `pxcor` and `pycor`, representing the spatial location (*i.e.*, coordinates) of the center of the patches.
See `help("worldMatrix-class")` for more details on the patch coordinates system. Turtles are defined by their coordinates `xcor` and `ycor` and data associated in the `agentMatrix` objects.
...
Functions can modify or used them to compute some value.
Functions available from the R software and from different packages can be used, as well as the functions from `NetLogoR`.
Most of the `NetLogoR` functions are a translation into the R language of the NetLogo's primitives (*i.e.*, "built-in commands and reporters").
...
```{r, eval=FALSE}
# create a world according to the given extent
...
The code can be sent directly by the observer or by a scheduler function (*e.g.*, see `SpaDES` package) which execute the code according to the schedule.
...
Turtles variables are stored by columns in the data of the `agentMatrix` object that represent them.
...
Several turtles variables built-in into NetLogo are created as the same time when creating turtles in `NetLogoR`: `xcor`, `ycor`, `who`, `heading`, `prevX`, `prevY`, `breed`, and `color`.
New turtle variables can be created:

```{r, eval=FALSE}
# now each turtle in t1 have a "sex" variable
...
You can either specify different breeds in the same turtles object or create two objects, one sheep and one wolf.
...
```{r, eval=FALSE}
# turtle 0 which was "sheep" became "wolf"
t2 <- NLset(turtles = t2, agents = turtle(t2, who = 0), var = "breed", val = "wolf")
```

Global variables are available for all agents.
Patches and turtles variables are available to other agents if they can be identified by their coordinates or some other variables.
...
However, you can still plot the agent positions in the world (*i.e.*, "view" in NetLogo) as well as any other plots (*e.g.*, the number of turtles through time) on the R plot interface.
...
The following steps describe how to build a general NetLogo type model in R using `NetLogoR`.
...
This can be done at any time in the code section but must appear before using any functions from a package.
...
You can visualize the world by plotting it.
As `plot` cannot handle `worldMatrix` objects, you need to convert it to a `Raster` object before: `plot(world2raster(nameWorld))`.

- Create the turtles (*i.e.*, moving agents) with the function `createTurtles()`.
You can visualize the turtles by plotting them on the world.
As `points` cannot handle `agentdMatrix` objects, you need to convert it to a `SpatialPointsDataFrame` object before: `points(turtles2spdf(nameTurtles), pch = 16)`.
...
The functions placed inside the for-loop or scheduler function will be iterated the number of time steps defined.
...
The visualization can be updated at each time step and/or new visualizations can be plotted at the end when the iterations are over.
Remember that plotting takes time to be executed and can slow down the model.
