...
This document is based on the [NetLogo Dictionary](https://ccl.northwestern.edu/netlogo/docs/dictionary.html). It lists all the entries in the NetLogo Dictionary and informs the best replacements to use in R. Are listed, functions from the NetLogoR package, as well as functions from the R software and of other packages (indicated as "nameFunction from namePackage"). In general, only the name of the functions are provided; to find how to use them, please refers to their documentation by using `help("nameFunction")`.

When no NeLogoR function was built to translate a NetLogo primitive and that, to our knowledge, no R function (or from other packages) could easily recreate it, we indicated "not implemented". The NetLogo primitives not implemented sould not prevent the users from building individual-based models involving patches or turtles.
...
* ask: not implemented. The agents running a given function are one of the function arguments that need to be provided.
...
* clear-turtles (ct): see NetLogoR dictionnary entry for clear-globals. If reassigning, `turtleAgentset <- noTurtles()` can be used to kill all the turtles in the agentset and replace it by an empty turtle agenset.
...
* patches-own: a new NLworld must be created with `createNLworld()` to create a new patch variable. This new world can then be stacked with others using `NLstack()` to create a single NLworldStack object .
...
* resize-world: create a new NLworld with `createNLworld()` and assign it to the object name of the previous world to overwrite it. Patches values from the previous world are then lost. To kill all the turtles, see the NetLogoR Dictionnary entry for clear-turtles.
...
* with-local-randomness: save the seed (e.g., `oldSeed <- .Random.seed`) and reuse it later with `set.seed(oldSeed)`.
