...
In NetLogoR, the world is a `NLworldMs` R object, either a `worldMatrix` or a `NLworldArray` R object. The world is a matrix, or a collection of matrices which represents a two dimensional landscape, divided into a grid of square patches (i.e., matrix cells) of resolution 1. 

There are two types of agents in NetLogoR: the patches and the turtles. Patches cannot move whereas turtles can move. Patches are defined by their coordinates `pxcor` and `pycor`, representing the spatial location of their center, as `matrix` objects. See `help("NLworld")` for more details on the patch coordinates system. Turtles are defined by their coordinates `xcor` and `ycor` and data associated as `SpatialPointDataFrame` objects. Patches coordinates are always integers but turtles coordinates can have decimals (i.e., a turtle can be positioned anywhere on a patch).
...
In NetLogoR, functions act on agents. Functions can transform agents or used them to compute some value. Functions available from the R software and different packages can be used, as well as the functions from NetLogoR. The NetLogoR functions are a translation into the R language of the NetLogo’s primitives (i.e., “built-in commands and reporters”). You can create new functions or wrap several existing functions into one. The rules to do so are imposed by the R language.  
...
In NetLogoR, one of the function argument indicates which agents (i.e., patches or turtles) are involved in the function. For example, in the `NLdist` function, the argument `agents` takes one patch coordinates and the argument `agents2` takes another patch coordinates.
...
To create a new patch variable, you create a `NLworld` and assign values. The name of the object to which the NLworld is assigned is treated as the name of the patch variable.
```{r, eval=FALSE}
pQuality <- createNLworld(minPxcor = 0, maxPxcor = 10, minPycor = 0, maxPycor = 10)
pQuality <- set(world = pQuality, agents = patches(pQuality), val = runif(n = length(pQuality), min = 0, max = 1)) # for all patches, assign a random value between 0 and 1
```

Several turtles variables built-in into NetLogo are created as the same time when creating turtles in NetLogoR: "xcor", "ycor", "who", "heading", "breed", and "color. New turtle variables can be created: 
```{r, eval=FALSE}
...
```{r, eval=FALSE}
t2 <- set(turtles = t2, agents = turtle(t2, who = 0), var = "breed", val = "wolf") # turtle 0 which was "sheep" became "wolf"
```
...
```{r, eval=FALSE}
of(world = pQuality, agents = patch(w1, c(0,0,0), c(0,1,2))) # reports the pQuality value of the patches [pxcor = 0, pycor = 0], [pxcor = 0, pycor = 1] and [pxcor = 0, pycor = 2]
```
...
NetLogo uses lists to store elements. In R, it is easier to use vectors when all the elements stored are numerical.
...
* Create a NLworld object with the function `createNLworld()` to create the world in which the agents will evolve. Defining the NLworld's extent in the function `createNLworld()` is similar as going in the "Interface" tab in NetLogo and clicking on the "Settings" button to change the model settings. 

* Define and assign values to the patches with the function `set()`. Now you can start visualizing the world by plotting it (an empty world cannot be plot) with the function `Plot(nameWorld)` from the package SpaDES. The plot functions of SpaDES are useful for this type of model.
...
* It is useful to test the different procedures individually with a small world and a few numbers of turtles to be sure the code is doing what we want it to do.
