...
* (Done) NLworldMs and agentMatrix to finish

* (Done) Add these new class as methods in the functions + unit tests.

* (Choose one way of defining the world/patches and the turtles, i.e., remove NLworlds and SPDF)

* Update functions documentation (e.g., input and output classes) + Programming guide and Dictionary

* Define plot functions for NLworldMs and agentMatrix

* (Done) Define convert functions, raster <-> NLworldMs and SPDF <-> agentMatrix. 
*Maybe need stronger unit tests, especially for raster2world() and world2raster()*



#### April 2016

Potential tasks to do:

* N) (Done) Build the Wolf-Sheep-Predation model (or another one) using SpaDES.

* M) Build new modele example(s) (mostly to help pinpoint slow and not-general enough functions).

* L) Code all the NL link functions.

* E) (Done) Remove "useless" parameters in signature functions (e.g., each time "world" is asked, it is to be used with the wrap function. "world" is not needed when torus = FALSE). Need to be changed in all dependent functions and tests associated. E.g., fd(), bk() and NLdist() but maybe others.

* G) Increase functions speed (e.g., better manage the outputs by removing lists)
...
* A) (Done) Make the functions clearTurtles(), clearAll() (this one is not written yet) and resizeWorld() work by figuring out the removing of R objects and environments when wrapping functions OR just explain in the dictionary or programming guide how to remove R objects and not building functions for that.
*Explained in the NLR dictionnary how to emulate these functions. They weren't that necessary to be implemented as functions as their replacements suggested are really easy, fast and straightforward to use.*
...
*I removed most of the for-loops where I could. For-loops are still remaining in towards() (turtle-functions.R), convertNLworld() (world-functions.R) and in the replacement method []<- for the signature NLworldStack (NLworlds-class.r). The function convertNLworld() could also be improved for speed regarding the use of resample(). I also improved the functions that I used in the two model examples (Butterfly-1 and Wolf-Sheep-Predation) and some others for the speed and to be more general (e.g., take different input formats)*

* H) (Done) Build a model with the NLR functions (e.g., take a NL example model and translate it in NLR). Document the steps of doing it in the programming guide in a "How to build a NLR model".
*I built two model examples: Butterfly-1 and Wolf-Sheep-Predation. Both use a for-loop for the time. The Wolf-Sheep-Predation could be written using SpaDES*

* I) (Done) Update the programming guide with the on-going changes.
*Up to date for the changes made until 2016-04-13*

* J) (Done) Complete the NLR dictionary with all the NL functions not defined in NLR because not needed or already in R or R packages.
*3 entries missing. An issue was opened about it*

* K) Code all the NL visual functions (e.g., plotting/drawing) and update the documents (i.e., dictionary and guide).
*I don't think this is necessary because a lot of the plotting functions exist and making a nice interface is not the point of using NetLogoR. Besides, it will be very slow and never as efficient as NetLogo*
