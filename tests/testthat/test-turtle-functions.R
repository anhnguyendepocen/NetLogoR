test_that("createTurtles works with different missing inputs",{
  ...
  t2 <- createTurtles(n = 10, coords = cbind(xcor = 0, ycor = 0))
  expect_identical(cbind(xcor = rep(0, 10), ycor = rep(0, 10)), t2@coords)
...
test_that("towards works",{
  ...
  # Patches to patch
  pTOp <- towards(world = w1, agents = patches(world = w1), agents2 = patches(world = w1))
  expect_equivalent(pTOp, rep(0, 100))
  pTOp <- towards(world = w1, agents = patches(world = w1), agents2 = patch(world = w1, x = 1, y = 1), torus = FALSE)
  expect_equivalent(pTOp[1], 180)
  pTOp <- towards(world = w1, agents = patches(world = w1), agents2 = patch(world = w1, x = 1, y = 1), torus = TRUE)
  expect_equivalent(pTOp[1], 0)

  # Patches to location
  pTOl <- towards(world = w1, agents = patches(world = w1), agents2 = cbind(x = 4, y = 8), torus = FALSE)
  expect_equivalent(pTOl[21], 90)
  pTOl <- towards(world = w1, agents = patches(world = w1), agents2 = cbind(x = 4, y = 8), torus = FALSE)
  expect_equivalent(pTOl[30], 270)
  pTOl <- towards(world = w1, agents = patches(world = w1), agents2 = cbind(x = 4, y = 8), torus = TRUE)
  ...
  ws <-NLstack(w1, w2)
  # Patches to patch
  pTOp <- towards(world = ws, agents = patches(world = ws), agents2 = patch(world = ws, x = 1, y = 1), torus = TRUE)
  expect_equivalent(pTOp[1], 0)
  # Patches to location
  pTOl <- towards(world = ws, agents = patches(world = ws), agents2 = cbind(x = 4, y = 8), torus = TRUE)
  expect_equivalent(pTOl[30], 90)
  # Patches to turtle
  pTOt <- towards(world = ws, agents = patches(world = ws), agents2 = t2, torus = TRUE)
  expect_equivalent(pTOt[95], 135)
  # Turtles to patch
  tTOp <- towards(world = ws, agents = t3, agents2 = patch(world = ws, x = 7, y = 2), torus = TRUE)
  expect_equivalent(tTOp[4], 0)
  # Turtles to location
  tTOl <- towards(world = ws, agents = t3, agents2 = cbind(x = 8, y = 4), torus = TRUE)
  expect_equivalent(tTOl[1], 270)
  # Turtles to turtle
...
test_that("downhill works",{
  ...
  expect_equivalent(t2@coords, cbind(xcor = 2, ycor = 3))
  expect_identical(t2@data$heading, 0)
  expect_equivalent(t3@coords, cbind(xcor = 1, ycor = 3))
  expect_identical(t3@data$heading, 315)
  ...
  expect_equivalent(t7@coords, cbind(xcor = 0, ycor = 1))
  expect_equivalent(t8@coords, cbind(xcor = 0, ycor = 4))
...
test_that("uphill works",{
  ...
  expect_equivalent(t2@coords, cbind(xcor = 2, ycor = 1))
  expect_identical(t2@data$heading, 180)
  expect_equivalent(t3@coords, cbind(xcor = 3, ycor = 1))
  expect_identical(t3@data$heading, 135)
  ...
  expect_equivalent(t7@coords, cbind(xcor = 2, ycor = 3))
  expect_equivalent(t8@coords, cbind(xcor = 2, ycor = 0))
  t9 <- createTurtles(n = 2, coords = cbind(xcor = c(1, 1), ycor = c(0, 1)))
...
test_that("inspect works",{
  ...
  df_t1.1 <- cbind.data.frame(who = 1, heading = 90, prevX = NA, prevY = NA, xcor = 2, ycor = 2)
...
test_that("moveTo works to move to turtles location",{
  t1 <- createTurtles(n = 4, coords = cbind(xcor = c(1,2,3,4), ycor = c(1,2,3,4)))
  t2 <- moveTo(turtles = t1, agents = t1[t1$who == 3,])
...
test_that("turtle works",{
  ...
  t1 <- createTurtles(n = 10, coords = randomXYcor(world = w1, n = 10), breed = c(rep("sheep", 5), rep("wolf", 5)))
  t2 <- turtle(t1, 3)
  expect_identical(t2@data$who, 3)
  expect_identical(t2@data$breed, "sheep")
  expect_equivalent(length(t2), 1)
  t3 <- turtle(t1, 3, "sheep")
  expect_identical(t2, t3)
  t4 <- turtle(t1, 9, "sheep")
  expect_equivalent(length(t4), 0)
  t5 <- turtle(t1, 9, "moose")
  expect_equivalent(length(t5), 0)
  t6 <- turtle(t1, who = c(3, 9))
  expect_identical(t6@data$who, c(3, 9))
  expect_identical(t6@data$breed, c("sheep", "wolf"))
  expect_equivalent(length(t6), 2)
  t7 <- turtle(t1, who = c(3, 9), breed = "sheep")
  expect_identical(t7@data$who, 3)
  expect_identical(t7@data$breed, "sheep")
  expect_equivalent(length(t7), 1)
...
test_that("layoutCircle works",{
  ...
  expect_identical(t2@data$heading, t3@data$heading)
  t4 <- layoutCircle(world = w1, turtles = turtle(turtles = t1, who = 0), radius = 3)
  expect_equivalent(t4@coords, cbind(xcor = 4.5, ycor = 7.5))
  expect_identical(t4@data$heading, 0)
  t5 <- layoutCircle(world = w1, turtles = turtle(turtles = t1, who = c(0, 1)), radius = 6, torus = FALSE)
  expect_equivalent(t5@coords, cbind(xcor = c(4.5, 4.5), ycor = c(10.5, -1.5)))
  expect_identical(t5@data$heading, c(0, 180))
