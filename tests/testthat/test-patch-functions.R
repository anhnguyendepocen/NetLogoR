...
test_that("distance works for patches", {
  ...
  dist <- NLdist(world = w1, agents1 = cbind(pxcor = 0, pycor = 0), agents2 = cbind(pxcor = 1, pycor = 1))
  expect_identical(as.numeric(dist), sqrt(1^2+1^2))
  dist <- NLdist(world = w1, agents1 = cbind(pxcor = 0, pycor = 0), agents2 = cbind(pxcor = c(0,0), pycor = c(1,9)))
  expect_identical(dist, c(1, 9))
  dist <- NLdist(world = w1, agents1 = cbind(pxcor = 0, pycor = 0), agents2 = cbind(pxcor = c(0,0), pycor = c(1,9)), torus = TRUE)
  expect_identical(dist, c(1, 1))
  dist <- NLdist(world = w1, agents1 = cbind(pxcor = c(0,0), pycor = c(0,1)), agents2 = cbind(pxcor = c(0,0), pycor = c(2,9)))
  expect_identical(dist, c(2, 8))
  dist <- NLdist(world = w1, agents1 = cbind(pxcor = c(0,0), pycor = c(0,1)), agents2 = cbind(pxcor = c(0,0), pycor = c(2,9)), allPairs = TRUE)
  ...
  w2[] <- runif(100)
  ws <- NLstack(w1, w2)
  dist <- NLdist(world = ws, agents1 = cbind(pxcor = 0, pycor = 0), agents2 = cbind(pxcor = 1, pycor = 1))
  expect_identical(as.numeric(dist), sqrt(1^2+1^2))
  dist <- NLdist(world = ws, agents1 = cbind(pxcor = 0, pycor = 0), agents2 = cbind(pxcor = c(0,0), pycor = c(1,9)))
  expect_identical(dist, c(1, 9))
  dist <- NLdist(world = ws, agents1 = cbind(pxcor = 0, pycor = 0), agents2 = cbind(pxcor = c(0,0), pycor = c(1,9)), torus = TRUE)
  expect_identical(dist, c(1, 1))
  dist <- NLdist(world = ws, agents1 = cbind(pxcor = c(0,0), pycor = c(0,1)), agents2 = cbind(pxcor = c(0,0), pycor = c(2,9)))
  expect_identical(dist, c(2, 8))
  dist <- NLdist(world = ws, agents1 = cbind(pxcor = c(0,0), pycor = c(0,1)), agents2 = cbind(pxcor = c(0,0), pycor = c(2,9)), allPairs = TRUE)
  expect_identical(dist[,1], c(2, 1))
  expect_identical(dist[,2], c(9, 8))

  w3 <- createNLworld(-5, 5, -10, -2)
  dist <- NLdist(world = w3, agents1 = cbind(pxcor = -2, pycor = -5), agents2 = cbind(pxcor = c(-1,5), pycor = c(-5, -5)), torus = TRUE)
...
test_that("distance works with turtles", {
  ...
  # Patches to turtles
  distPT <- NLdist(world = w1, agents1 = cbind(pxcor = 2, pycor = 3), agents2 = t1)
  expect_identical(distPT, c(sqrt(1^2+2^2), 1, 1, sqrt(1^2+2^2)))
  distPT <- NLdist(world = w1, agents1 = cbind(pxcor = 8, pycor = 1), agents2 = t1)
  expect_identical(distPT[1], 7)
  distPT <- NLdist(world = w1, agents1 = cbind(pxcor = 8, pycor = 1), agents2 = t1, torus = TRUE)
  expect_identical(distPT[1], 3)

  # Turtles to patches
  distTP <- NLdist(world = w1, agents1 = t1, agents2 = cbind(pxcor = 2, pycor = 3))
  expect_identical(distTP, c(sqrt(1^2+2^2), 1, 1, sqrt(1^2+2^2)))

  # Turtles to turtles
  distTT <- NLdist(world = w1, agents1 = t1, agents2 = t1)
  expect_equivalent(distTT, rep(0, 4))
  distTT <- NLdist(world = w1, agents1 = t1, agents2 = t1, allPairs = TRUE)
  ...
  w2[] <- runif(100)
  ws <- NLstack(w1, w2)
  distPT <- NLdist(world = ws, agents1 = cbind(pxcor = 8, pycor = 1), agents2 = t1, torus = TRUE)
  expect_identical(distPT[1], 3)
  distTP <- NLdist(world = ws, agents1 = t1, agents2 = cbind(pxcor = 2, pycor = 3))
  expect_identical(distTP, c(sqrt(1^2+2^2), 1, 1, sqrt(1^2+2^2)))
  distTT <- NLdist(world = ws, agents1 = t1, agents2 = t1, allPairs = TRUE)
  expect_equivalent(distTT[1,2], sqrt(1^1+1^1))
})
...
test_that("patchDistDir works with patches", {
  w1 <- createNLworld(0, 9, 0, 9)
  ...
  p2 <- patchDistDir(world = w1, agents = cbind(pxcor = c(0, 1, 3), pycor = c(0, 1, 5)), dist = 3, angle = 315, torus = TRUE)
  ...
  p2 <- patchDistDir(world = ws, agents = cbind(pxcor = c(0, 1, 3), pycor = c(0, 1, 5)), dist = 3, angle = 315, torus = TRUE)
...
test_that("patchDistDir works with turtles", {
  w1 <- createNLworld(0, 9, 0, 9)
