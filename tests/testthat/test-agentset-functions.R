...
test_that("inRadius works",{
  ...
  t2 <- inRadius(agents1 = patch(w1, x = 0, y = 0), radius = 1, agents2 = t1, world = w1)
  expect_identical(t2[[1]], turtle(t1, 0))
  t3 <- inRadius(agents1 = patch(w1, x = 0, y = 0), radius = 2, agents2 = t1, world = w1)
  expect_identical(t3[[1]], turtle(t1, c(0, 1)))
  t4 <- inRadius(agents1 = patch(w1, x = 0, y = 0), radius = 2, agents2 = t1, world = w1, torus = TRUE)
...
test_that("inCone works",{
  ...
  t6 <- inCone(turtles = turtle(t1, who = 1), radius = 1, angle = 181, agents2 = patch(w1, x = 0, y = 0), world = w1, torus = FALSE)
  expect_equivalent(t6[[1]], noPatches())
  t7 <- inCone(turtles = turtle(t1, who = c(1,2)), radius = 1, angle = 181, agents2 = patch(w1, x = 2, y = 2), world = w1, torus = FALSE)
