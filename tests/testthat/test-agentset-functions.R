...
test_that("nOf works with NLworldMs and agentMatrix",{
...
test_that("oneOf works with NLworldMs and agentMatrix",{
  w1 <- createNLworldMatrix(minPxcor = 0, maxPxcor = 4, minPycor = 0, maxPycor = 4)
  p1 <- oneOf(agents = patches(world = w1))
  expect_equivalent(nrow(p1), 1)
  p11 <- as.matrix(merge(p1, patches(world = w1)))
...
test_that("minNof works with NLworldMs and agentMatrix",{
  ...
  t1 <- createTurtlesAM(n = 10, coords = randomXYcor(world = w1, n = 10), heading = 10:1)
  t2 <- minNof(agents = t1, n = 5, var = "heading")
  expect_equivalent(t2, turtle(t1, who = c(5,6,7,8,9)))
  t3 <- minNof(agents = t1, n = 1, var = "heading")
  expect_equivalent(t3, turtle(t1, who = 9))
...
test_that("inRadius2 works with NLworldMs and agentMatrix",{
  # Patches to patches
  w1 <- createNLworldMatrix(minPxcor = 0, maxPxcor = 4, minPycor = 0, maxPycor = 4)
  ...
  p3 <- inRadius2(agents = patch(w1, 0, 0), radius = 2, agents2 = patch(w1, 0, 0), world = w1)
  expect_equivalent(p3[p3[,"id"] == 1, c("pxcor", "pycor"), drop = FALSE], patch(w1, 0, 0))
  expect_equivalent(nrow(p3), 1)
  p4 <- inRadius2(agents = patches(w1), radius = 10, agents2 = patches(w1), world = w1)
  expect_equivalent(p4[p4[,"id"] == 1, c("pxcor", "pycor")], patches(w1))
  expect_equivalent(length(unique(p4[,"id"])), nrow(patches(w1)))
  expect_equivalent(p4[p4[,"id"] == 1, c("pxcor", "pycor")], p4[p4[,"id"] == 2, c("pxcor", "pycor")])

  # Patches to turtles
  t1 <- createTurtlesAM(n = 5, coords = cbind(xcor = 0:4, ycor = 0:4))
  t2 <- inRadius2(agents = patch(w1, x = 0, y = 0), radius = 1, agents2 = t1, world = w1)
  ...
  t6 <- inRadius2(agents = patches(w1), radius = 10, agents2 = t1, world = w1)
  expect_equivalent(t6[t6[,"id"] == 1, "who"], t1@.Data[,"who"])
  expect_equivalent(t6[t6[,"id"] == 25, "who"], t1@.Data[,"who"])
  t7 <- inRadius2(agents = patches(w1), radius = 10, agents2 = t1, world = w1, torus = TRUE)
  ...
  p8 <- inRadius2(agents = turtle(t1, c(0, 4)), radius = 1, agents2 = patches(w1), world = w1, torus = TRUE)
  expect_equivalent(nrow(merge(p8[p8[,"id"] == 1, c("pxcor", "pycor")], cbind(pxcor = c(0, 0, 0, 1, 4), pycor = c(4, 1, 0, 0, 0)))), nrow(p8[p8[,"id"] == 1,]))
  expect_equivalent(nrow(merge(p8[p8[,"id"] == 2, c("pxcor", "pycor")], cbind(pxcor = c(0, 3, 4, 4, 4), pycor = c(4, 4, 4, 3, 0)))), nrow(p8[p8[,"id"] == 2,]))
  p9 <- inRadius2(agents = turtle(t1, 0), radius = 1, agents2 = patch(w1, 4, 4), world = w1)
  expect_equivalent(NROW(p9), 0)

  # Turtles to turtles
  t8 <- inRadius2(agents = turtle(t1, 0), radius = 1, agents2 = t1, world = w1)
  expect_equivalent(t8[t8[,"id"] == 1, "who"], 0)
  t9 <- inRadius2(agents = turtle(t1, 0), radius = 2, agents2 = t1, world = w1)
  expect_equivalent(t9[t9[,"id"] == 1, "who"], c(0, 1))
  t10 <- inRadius2(agents = turtle(t1, 0), radius = 2, agents2 = t1, world = w1, torus = TRUE)
  expect_equivalent(t10[t10[,"id"] == 1, "who"], c(0, 1, 4))
  t11 <- inRadius2(agents = t1, radius = 10, agents2 = t1, world = w1)
  expect_equivalent(t11[t11[,"id"] == 1, "who"], c(0, 1, 2 , 3, 4))

  # Works without the world provided when torus = FALSE
  p1 <- inRadius2(agents = patch(w1, 0, 0), radius = 2, agents2 = patches(w1))
  expect_equivalent(p1[p1[,"id"] == 1, c("pxcor", "pycor")], cbind(pxcor = c(0, 0, 1, 0, 1, 2), pycor = c(2, 1, 1, 0, 0, 0)))
  p2 <- inRadius2(agents = patches(w1), radius = 2, agents2 = patch(w1, 0, 0))
  expect_equivalent(p2[p2[,"id"] == 11, c("pxcor", "pycor"), drop = FALSE], cbind(pxcor = 0, pycor = 0))
  p3 <- inRadius2(agents = patch(w1, 0, 0), radius = 2, agents2 = patch(w1, 0, 0))
  expect_equivalent(p3[p3[,"id"] == 1, c("pxcor", "pycor"), drop = FALSE], patch(w1, 0, 0))
  expect_equivalent(nrow(p3), 1)
  p4 <- inRadius2(agents = patches(w1), radius = 10, agents2 = patches(w1))
  expect_equivalent(p4[p4[,"id"] == 1, c("pxcor", "pycor")], patches(w1))
  expect_equivalent(length(unique(p4[,"id"])), nrow(patches(w1)))
  expect_equivalent(p4[p4[,"id"] == 1, c("pxcor", "pycor")], p4[p4[,"id"] == 2, c("pxcor", "pycor")])
  t2 <- inRadius2(agents = patch(w1, x = 0, y = 0), radius = 1, agents2 = t1)
  expect_equivalent(t2[t2[,"id"] == 1, "who"], 0)
  t3 <- inRadius2(agents = patch(w1, x = 0, y = 0), radius = 2, agents2 = t1)
  expect_equivalent(t3[t3[,"id"] == 1, "who"], c(0, 1))
  t5 <- inRadius2(agents = patches(w1), radius = 1, agents2 = t1)
  expect_equivalent(t5[t5[,"id"] == 5, "who"], 4)
  expect_equivalent(t5[t5[,"id"] == 9, "who"], 3)
  t6 <- inRadius2(agents = patches(w1), radius = 10, agents2 = t1)
  expect_equivalent(t6[t6[,"id"] == 1, "who"], t1@.Data[,"who"])
  expect_equivalent(t6[t6[,"id"] == 25, "who"], t1@.Data[,"who"])
  ...
  expect_equivalent(NROW(p9), 0)
  t8 <- inRadius2(agents = turtle(t1, 0), radius = 1, agents2 = t1)
  expect_equivalent(t8[t8[,"id"] == 1, "who"], 0)
  t9 <- inRadius2(agents = turtle(t1, 0), radius = 2, agents2 = t1)
  expect_equivalent(t9[t9[,"id"] == 1, "who"], c(0, 1))
  t11 <- inRadius2(agents = t1, radius = 10, agents2 = t1, world = w1)
  expect_equivalent(t11[t11[,"id"] == 1, "who"], c(0, 1, 2 , 3, 4))
})
...
test_that("inCone works with agentMatrix",{
  w1 <- createNLworldMatrix(minPxcor = 0, maxPxcor = 4, minPycor = 0, maxPycor = 4)
  ...
  expect_identical(t4[t4[,"id"] == 1, c("pxcor", "pycor")], patch(w1, x = c(0,0,1), y = c(1, 0, 0)))
  t5 <- inCone(turtles = turtle(t1, who = 0), radius = 1, angle = 181, agents = patches(w1), world = w1, torus = TRUE)
  expect_equivalent(nrow(merge(t5[t5[,"id"] == 1, c("pxcor", "pycor")], patch(w1, x = c(0,0,1, 4), y = c(1, 0, 0, 0)))), 4)
  t6 <- inCone(turtles = turtle(t1, who = 1), radius = 1, angle = 181, agents = patch(w1, x = 0, y = 0), world = w1, torus = FALSE)
  expect_equivalent(nrow(t6), 0)
  t7 <- inCone(turtles = turtle(t1, who = c(1,2)), radius = 1, angle = 181, agents = patch(w1, x = 2, y = 2))
  expect_equivalent(nrow(t7[t7[,"id"] ==1,]), 0)
  expect_equivalent(t7[t7[,"id"] == 2, c("pxcor", "pycor"), drop = FALSE], patch(w1, x = 2, y = 2))
...
test_that("set works",{
  ...
  w1 <- NLset(world = w1, agents = patch(w1, c(0,1), c(0,1)), val = 100)
  expect_equivalent(w1[0,0], 100)
  expect_equivalent(w1[1,1], 100)
  ...
  w33 <- NLset(world = w3, agents = patch(w3, c(0,1), c(0,1)), var = "w1", val = 100)
  expect_equivalent(w33[0,0][1], 100)
  expect_equivalent(w33[1,1][1], 100)
  ...
  w33 <- NLset(world = w3, agents = patch(w3, c(0,1), c(0,1)), var = c("w2", "w1"), val = cbind(w2 = 0, w1 = 100))
  expect_equivalent(w33[0,0], cbind(w1 = 100, w2 = 0))
  expect_equivalent(w33[1,1], cbind(w1 = 100, w2 = 0))
  w33 <- NLset(world = w3, agents = patch(w3, c(0,1), c(0,1)), var = c("w2", "w1"), val = cbind.data.frame(w2 = 0, w1 = 100))
  expect_equivalent(w33[0,0], cbind(w1 = 100, w2 = 0))
  expect_equivalent(w33[1,1], cbind(w1 = 100, w2 = 0))
